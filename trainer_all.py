...
from dataset import create_from_patch2patch
...
def get_loss_function(loss_type):
    ...
    elif loss_type == "MaskedMSELoss":
        from models.unetPlusPlus32_attention import MaskedMSELoss
        return MaskedMSELoss()

    elif loss_type == "MaskedCenterMSELoss_patch32":
        # only for patch size 32
        from models.unetPlusPlus32_attention import MaskedCenterMSELoss
        return MaskedCenterMSELoss()

    elif loss_type == "MaskedCenterMSELoss_patch64":
        # only for patch size 64
        from models.unetPlusPlus64_attention import MaskedCenterMSELoss
        return MaskedCenterMSELoss()
...
def main_train(config, logger, writer, device, stage=''):
    ...
    train_data, test_data = create_from_patch2patch(
        data_paths, features_dir, targets_dir, mode="train/val", is_norm=False)
    ...
    depth_model, total_train_step = trainer(model=depth_model, train_dataloader=train_dataloader,
                                            test_dataloader=test_dataloader,
                                            optimizer=optimizer, loss_fn=loss, device=device, epochs=epochs,
                                            writer=writer, logger=logger, global_step=0, stage=stage,
                                            save_best=False)
...
def main(config):
    ...
    trained_model, _ = main_train(
        config["training"], logger, writer, device, stage="training"
    )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='configs/training_config_all.yaml', help='config path')
